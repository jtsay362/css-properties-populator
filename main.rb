      ...
      out.write <<-eos
{
  ...
  def parse_file(mdn_file_path, web_platform_file_path, out)
    ...
    # Prefer the CSS color property name, which is indistuishable with the
    # lower_keyword analyzer
    if name == '<color>'
      return
    end
